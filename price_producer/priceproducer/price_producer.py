...
class KafkaProducerManager:
    ...
    async def asyncproduce(self):
        await asyncio.gather(*[asyncio.create_task(self._producer.send(topic, value=gen())) for topic, gen in self._topics_and_gens])

    def produce(self):
        for topic, gen in self._topics_and_gens:
            self._producer.send(topic, value={'value':gen(), 'time': str(datetime.datetime.now())})
